...
transformers_time = time.time()

trans_input = processor(images=image_np, text=question, return_tensors="pt")
trans_prediction = model.generate(**trans_input)

print(processor.decode(trans_prediction[0], skip_special_tokens=True))
print("Transformers Inference Time:", round(time.time() - transformers_time, 3))